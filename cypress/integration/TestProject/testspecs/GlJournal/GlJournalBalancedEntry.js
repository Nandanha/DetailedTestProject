...
it('Gljournal PageTest for BalancedEntry creation test',() => {
...
TrinityGljournalPage.rollbackEntry();
TrinityGljournalPage.deleteEntry();  
    

/*
	cy.get('[comp-id="483"] > .ag-cell-wrapper > .ag-selection-checkbox > .ag-icon-checkbox-unchecked').click();
	cy.get('[comp-id="491"]').invoke('text').then(sometext =>cy.log(sometext));
	const searchkey= sometext;
	log(searchkey);
*/
})